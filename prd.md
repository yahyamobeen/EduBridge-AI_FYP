...
|---------|------|--------|---------|
| 0.3.10 | 2026-08-17 | EduBridge AI Team | **The parental-consent gate is passable for the first time (KAN-21).** Transactional email is resolved to **SendGrid** (free tier): verification links, 2FA codes, password resets and the guardian invite all deliver to any inbox, where the previous default (`logging`) sent nothing and the Resend sandbox reached only the account owner's own address — so **a real parent could never be contacted**. The guardian invitation is now actually sent; until this revision §4.3's gate minted an invitation token and discarded it while the interface said the parent had been emailed, which meant **no Class 9–10 student could complete onboarding at all**. ⚠️ **This does not close §4.3's remaining weakness**: a student still supplies the address themselves, so satisfying one's own gate with a throwaway parent account remains possible and is still an open item. No requirement changed; a requirement started being met. |
| 0.3.9 | 2026-08-17 | EduBridge AI Team | **FR-A8 gains its screen, and its Edge clause is amended rather than quietly contradicted.** The settings surface replaces the placeholder every role's sidebar pointed at. FR-A8 previously required that the stored language preference and the interface locale "are distinct settings and must be presented as such"; the built screen deliberately **links** them behind one control, because two language controls on one page is a usability defect in a product whose users are choosing between Urdu and English. The requirement is rewritten to say so. Board and class render **read-only** — `class_level` is the §4.3 gate input, so an editable field would let a Class 9 student escape the gate permanently. |
| 0.3.8 | 2026-08-17 | EduBridge AI Team | **FR-A8 is built, and doing so exposed that the stored language preference had nowhere to live for three roles out of four.** §4.2 has granted "manage own account" to every role since v0.1.0, but `language_pref` was a column on the student profile, so the lookup behind every outgoing email returned nothing for teachers, parents and administrators — **none of them could ever receive Urdu mail**, and the requirement was unmeetable rather than unimplemented. The preference now belongs to the account. Also records what a user may change about themselves — name and language, nothing else — and why `class_level`, `board` and `student_group` are not on that list. |
...
1. Parent receives an invite email, triggered by their child entering the parent's address (§4.3). Transactional mail — invite, verification links and 2FA codes — is delivered via **SendGrid** (free tier), which sends to any inbox (v0.3.10). ⚠️ The invite was **specified from v0.1.0 and not actually sent until v0.3.10**: the token was minted and discarded while the interface reported the parent had been emailed, so this step of the flow was unreachable and the Class 9–10 gate could not be cleared by anyone.
...
- **FR-A2a (P0):** **`ADMIN` does not use the public identity surface.** One requirement with two halves, because they are the same rule applied to the two ends of the account lifecycle.
  - **Not self-registrable.** Public registration accepts `STUDENT`, `TEACHER` and `PARENT` only; an administrator is provisioned by an operator with direct database access, never through the API. *AC:* a registration request naming `admin` is rejected with a validation error, and the restriction is visible in the published interface schema; the database independently refuses an administrator row to the application role, so a future endpoint that skips the check cannot create one. *Edge:* an operator promoting a user to administrator is unaffected, which is the only intended path.
  - **Segregated sign-in.** Administrators authenticate at a **separate endpoint**, reached through an **unlisted path** rather than the public sign-in page. *AC:* an administrator submitting correct credentials at the public login is refused, and a non-administrator is refused at the administrator endpoint. **Both refusals are byte-identical to a wrong password — same status, same code, same message, same argon2 cost** — so neither endpoint can be used to discover which addresses are administrators (§6.11 forbids revealing an account fact by body, status code or timing). *Edge:* the second factor, refresh and logout are **deliberately shared** with the ordinary flow; the challenge token is already bound to the user, so segregating them would add no security and would fork a tested path.
  - ⚠️ **The unlisted path is not an access control.** It is an unlisted door: it keeps the administrator entrance off the public site, and nothing more. The endpoint's role check is the lock, and it holds whether or not the path is known.
...
- **FR-A8 (P1):** **Manage own account.** *As any user, I want to change my password, my language and my details from inside my account, so that I am not forced through a reset email to change something I already know.* *AC:* a signed-in user can change their password after re-authenticating with the current one; the **stored language preference** can be changed and governs the language of subsequent outgoing email; own second-factor **status** is visible but the secret never is; everything is scoped to the acting user. *Role:* all. *Deps:* §4.2. *Edge:* the stored language preference and the interface locale are changed **together, by one control** — see the amendment below. *(Added in v0.3.5 — §4.2 has granted this to every role since v0.1.0, but no requirement described it and `tdd.md` routed nothing for it. Built in v0.3.8.)*

  > **The stored preference had nowhere to live for three roles out of four, and that was invisible until this was built (v0.3.8).** `language_pref` was a column on `student_profile`, so the acceptance criterion "governs the language of subsequent outgoing email" could not be met by a teacher, a parent or an administrator: the lookup behind every outgoing mail LEFT JOINed that table, returned `NULL` for them, and fell back to English for ever. Measured on the live administrator account before the fix — `language_pref -> None`. Migration `20260816200000` moves the column to `app_user`, which is now the **single source of truth** for both outgoing email and `MeResponse.profile.language_pref`; `student_profile.language_pref` is retained but read by nothing.
  >
  > **AMENDED IN v0.3.9, WHEN THE SCREEN WAS BUILT.** The Edge clause above previously read that the stored preference and the interface locale "are distinct settings and must be presented as such". They are distinct MECHANISMS and that has not changed — the stored preference is an API write that governs outgoing email, the interface locale is a URL segment — but presenting them as two controls asks a user to answer the same question twice and to understand a distinction that exists for the system's benefit, not theirs. The settings screen therefore offers **one** control, which writes the preference and then changes the route. The requirement is amended rather than the screen, because the screen is right.
  >
  > ⚠️ The consequence for whoever implements this: the two value sets are NOT the same strings. `ur-Latn` on the web is `roman_ur` in the API, so the control maps between them (`toApiLanguage` / `fromApiLanguage`) rather than passing the value straight across. And the API write must happen FIRST: navigating first would leave the user reading a page in a language the server does not think they chose.

  > **What a user may change about themselves is `full_name` and `language_pref`, and nothing else.** `class_level` is the input §4.3's parental-consent gate reads, and `board` and `student_group` scope every progress record a student has — so all three are set at registration and are unwritable at the database, not merely absent from the request model (§6.8, finding B4).
