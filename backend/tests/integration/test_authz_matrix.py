...
def _provisioned_admin(db) -> str:
    ...
    `service_conn` is NOT an alternative: it is a separate connection in a
    separate transaction, so a row inserted there is invisible to `db` and both
    roll back.
    """
    admin = db.execute(
