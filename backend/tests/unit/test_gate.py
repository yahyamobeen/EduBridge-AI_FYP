...
class TestIsGuardianGatePending:
    def test_class_9_student_without_verified_guardian_is_gated(self):
        assert (
            is_guardian_gate_pending(is_student=True, class_level=9, guardian_status=None) is True
        )

    @pytest.mark.parametrize("status", [None, "pending", "revoked"])
    def test_anything_other_than_verified_holds_the_gate(self, status):
        assert (
            is_guardian_gate_pending(is_student=True, class_level=9, guardian_status=status) is True
        )

    def test_verified_opens_the_gate(self):
        assert (
            is_guardian_gate_pending(is_student=True, class_level=9, guardian_status="verified")
            is False
        )
