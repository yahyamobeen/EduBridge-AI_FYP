...
|---------|------|--------|---------|
| 0.3.8 | 2026-08-16 | EduBridge AI Team | **Transactional email provider resolved to SendGrid (free tier).** The `EmailSender` seam in `app/auth/email.py` gains **`SendGridEmailSender`** alongside the dormant `ResendEmailSender`; the factory returns it when `EMAIL_PROVIDER=sendgrid`. `Settings` gains `SENDGRID_API_KEY` and a `EMAIL_PROVIDER=sendgrid requires EMAIL_FROM` guard. The `email` extra adds `sendgrid>=6.11.0`. `EMAIL_PROVIDER=logging` remains the safe default and stays refused in production. The guardian invite email (KAN-21) is now actually delivered: `guardian_invite()` builds the confirm URL, renders the email and queues it to the parent (§3.1, §8.2). |
...
| `EMAIL_PROVIDER` | `logging` | transactional email provider: `logging` (dev/CI) | `sendgrid` (production, chosen v0.3.8). `logging` is refused in production |
| `SENDGRID_API_KEY` | — | SendGrid Web API key (`SG.…`); required when `EMAIL_PROVIDER=sendgrid` |
| `EMAIL_FROM` | — | verified SendGrid single-sender address; required for both `sendgrid` and `resend` |
